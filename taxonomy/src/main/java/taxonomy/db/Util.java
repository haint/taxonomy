...
public class Util
{
   static void insertGlossary(StringBuilder b, String line, Connection con) throws Exception
   {
      ...
      if (!name_vn.equals(" "))
      {
         ...
         ResultSet rs = st.executeQuery("SELECT Max(ID)  FROM Locales");
         rs.next();
      ...
      String gso2, String table) throws Exception
   {
      ...
      if (!value2.equals(" ") && !value2.isEmpty())
      {
         ...
         ResultSet tmp = con.createStatement().executeQuery("Select Max(ID) from Variant");
         if (tmp.next())
      ...
      if (!rs.next())
      {
         if ("GENUS".equalsIgnoreCase(table))
         {
            ...
            ResultSet tmp = con.createStatement().executeQuery("Select Max(ID) FROM Genus");
            tmp.next();
            genId = tmp.getInt(1);
         }
         else if ("Species".equalsIgnoreCase(table))
         {
            ...
            ResultSet tmp = con.createStatement().executeQuery("Select Max(ID) FROM Species");
            tmp.next();
            genId = tmp.getInt(1);
         }
      }
   ...
   static String buildFamilyIds(Connection con, String Fami, String Fam2) throws Exception
   {
      ...
      else
      {
         ...
         {
            con.createStatement().executeUpdate("Insert into Family values(NULL, 1, '" + Fami + "', NULL, NULL, NULL)");
            ResultSet tmp = con.createStatement().executeQuery("SELECT Max(ID)  FROM Family");
            tmp.next();
            id = tmp.getInt(1);
         }
         else
   ...
   static String buildIndexIds(Connection con, String Inde) throws Exception
   {
      ...
      for (int i = 0; i < indecies.length; i++)
      {
         ...
         if (!rs.next())
         {
            ...
            ResultSet tmp = con.createStatement().executeQuery("Select Max(ID) from Index");
            tmp.next();
            id = tmp.getInt(1);
         }
         else
